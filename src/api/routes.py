...
def admin_login():
    ...
    if checkEmail is not None and bcrypt.checkpw(unSaltPass, checkEmail.password.encode('utf-8')) and checkEmail.role == "Admin":
        ...
        set_access_cookies(response, admin_token, max_age=max_age_seconds )
...
def login():
    ...
    if checkEmail is not None and bcrypt.checkpw(unSaltPass, checkEmail.password.encode('utf-8')):
        ...
        set_access_cookies(res, access_token, max_age=max_age_seconds)
...
@jwt_required()
def get_label():
    cases_included = request.json.get("cases", [])
