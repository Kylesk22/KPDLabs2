...
export const SingleOrder = props => {
    ...
    const downloadCase = () => {
        //   const link = document.createElement('a');
          console.log(stlFile)}
    //       for (let stl in stlFile) {
    //         console.log(stl)
    //       link.href = stl.scan;
    //       link.setAttribute('download', stl.scan);
    ...
    function downloadSTLFromBlob(blobUrl, fileName) {
        ...
        fetch(blobUrl)
            ...
            .then(blob => {
                ...
                const link = document.createElement('a');
                link.href = blobUrl;
                link.download = fileName; // Set the download attribute to specify the file name
        ...
        .then((res)=> {
            if (res.ok) {
                ...
                .then((data)=>{
                    ...
                    const numberArray = returnedTeeth
                    .replace(/[{}"]/g, '')   // remove { } "
                    .split(',')
                        ...
                        <div>
                            ...
                            <div style={{ width: "400px", height: "500px", border: "2px solid black", borderRadius: "5px", overflowY: "scroll" }} ref={logRef}>
                                <ul style={{backgroundColor: "white", color:"black"}}>
                                    {Array.isArray(log) && log.filter(item => !item.toLowerCase().includes("internal_kpd_note")).map((item, index) => (
